...
def _write_binary_matrix(filename, array):
...
def _create_chunk(prefix, random_state):
  ...
  _write_binary_matrix("%s-dat.mat" % prefix, images)

  # Create the class label file.
  class_labels = random_state.choice(range(5), size=(NUM_IMAGES))
  _write_binary_matrix("%s-cat.mat" % prefix, class_labels.astype("int32"))
  ...
  _write_binary_matrix("%s-info.mat" % prefix, np.array(info).T.astype("int32"))
