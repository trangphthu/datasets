  ...
  def __init__(self,
    ...
    Args:
      ...
      label_column: `string`, name of the column in the tsv file corresponding
        to the label
      label_feature_name: `string`, name of the label feature
      label_feature_type: `tfds.features.FeatureConnector`, the type of the
        label feature
      ...
      dummy_label: dummy value of the label to use for the test set
    ...
    self.label_feature_type = label_feature_type
...
class Glue(tfds.core.GeneratorBasedBuilder):
  ...
  BUILDER_CONFIGS = [
      GlueConfig(
          ...
          label_feature_name="is_acceptable",
          # Acceptability judgment label (0=unacceptable, 1=acceptable).
          label_feature_type=tfds.features.ClassLabel(num_classes=2),
          dummy_label=-1,
      ...
      GlueConfig(
          ...
          label_feature_name="are_equivalent",
          # Semantic equivalency (0=no, 1=yes).
          label_feature_type=tfds.features.ClassLabel(num_classes=2),
          dummy_label=-1,
      ...
      GlueConfig(
          ...
          label_feature_name="is_duplicate",
          # Label (0=not duplicate, 1=duplicate).
          label_feature_type=tfds.features.ClassLabel(num_classes=2),
          dummy_label=-1,
      ...
      GlueConfig(
          ...
          label_feature_name="similarity_score",
          # Similarity score (1=different, 5=same)
      ...
      GlueConfig(
          ...
          label_feature_type=tfds.features.ClassLabel(
      ...
      GlueConfig(
          ...
          description="""\
            ...
            new examples derived from fiction books
            that was shared privately by the authors of the original
            corpus. While the included training set is balanced between two classes, the test set is imbalanced
            between them (65% not entailment). Also, due to a data quirk, the development set is adversarial:
            hypotheses are sometimes shared between training and development examples, so if a model memorizes the training examples, they will predict the wrong label on corresponding development set
            example. As with QNLI, each example is evaluated separately, so there is not a systematic correspondence between a model's score on this task and its score on the unconverted original task. We
  ...
  def _info(self):
    ...
    features[self.builder_config
             .label_feature_name] = self.builder_config.label_feature_type
      ...
      with tf.io.gfile.GFile(data_file) as f:
        ...
        for row in reader:
          ...
          if self.builder_config.label_column in row:
            example[self.builder_config.label_feature_name] = process_label(
                row[self.builder_config.label_column])
          else:
            example[self.builder_config.label_feature_name] = process_label(
                self.builder_config.dummy_label)
          yield example
      ...
      with tf.io.gfile.GFile(mrpc_files["test"]) as f:
        ...
        for row in reader:
          yield {
              ...
              "are_equivalent": self.builder_config.dummy_label
          }
    else:
